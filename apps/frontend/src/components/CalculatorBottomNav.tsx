...
export const CalculatorBottomNav: React.FC<CalculatorBottomNavProps> = ({
  ...
  const isRtlLang = locale === 'fa' || locale === 'ar'
  ...
  return (
    <div className="flex-shrink-0 bg-surface border-t border-border-light shadow-lg pt-4 pb-10 px-1 relative z-50">
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
        {/* Left side: Total value */}
        <div className="flex flex-col items-center sm:items-start">
          <p className="text-md text-text-secondary">{t('total')}</p>
          ...
          <div className="relative flex-1 sm:flex-none" ref={menuRef}>
            <div className="flex gap-1">
              ...
              <Button
                ...
                className={`flex-1 sm:flex-none ${
                  isRtlLang ? 'rounded-l-none' : 'rounded-r-none'
                }`}
              ...
              <Button
                ...
                onClick={toggleMenu}
                className={`px-2 ${
                  isRtlLang ? 'rounded-r-none border-r' : 'rounded-l-none border-l'
                } border-white/20`}
                disabled={itemCount === 0 || isGeneratingPDF}
            ...
            {showLangMenu && (
              <div
                className={`absolute bottom-full mb-2 bg-surface border border-border-light rounded-lg shadow-xl overflow-hidden min-w-40 z-50 ${
                  isRtlLang ? 'left-0' : 'right-0'
                }`}
                role="menu"
