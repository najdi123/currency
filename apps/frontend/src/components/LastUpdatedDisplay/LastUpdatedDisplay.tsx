...
export const LastUpdatedDisplay = ({
  ...
  useEffect(() => {
    ...
    const pYear = parseInt(persianParts.find(p => p.type === 'year')?.value || '1404')
  ...
  return (
    ...
    {/* Action Buttons - Shows when dropdown is active */}
                  {activeDropdown && (
                    <div className="absolute bottom-16 right-0  mt-1 flex gap-2 z-10">
                      <button
                  ...
                  >
                    <select
                      ...
                      dir="ltr"
                    >
                      {Array.from({ length: 50 }, (_, i) => 1404 - i).map(year => (
