    ...
    if (response.data) {
      if (Array.isArray(response.data)) {
        ...
        if (response.data.result) {
          ...
          console.log(
            `   Result is array: ${Array.isArray(response.data.result)}`,
          );
          ...
          if (response.data.result.data) {
            ...
            console.log(
              `   Data is array: ${Array.isArray(response.data.result.data)}`,
            );
            if (Array.isArray(response.data.result.data)) {
              console.log(
                `   Data array length: ${response.data.result.data.length}`,
              );
              console.log(`\n📋 First 3 data items:`);
          ...
          if (response.data.result.list) {
            ...
            console.log(
              `   List is array: ${Array.isArray(response.data.result.list)}`,
            );
            if (Array.isArray(response.data.result.list)) {
              console.log(
                `   List array length: ${response.data.result.list.length}`,
              );
              console.log(`\n📋 First 3 list items:`);
...
async function main() {
  ...
  results.push({
    name: "Gold-Currency-Coin",
    url: "/common/gold-currency-coin",
    ...result,
  });
  ...
  results
    ...
    .forEach((r) => {
      console.log(
        `   ✅ ${r.name.padEnd(30)} ${r.url.padEnd(35)} (${r.count} items)`,
      );
    });
