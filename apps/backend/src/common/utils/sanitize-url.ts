...
export function sanitizeUrl(url: string): string {
  ...
  try {
    ...
    const urlObj = url.includes('://') ? new URL(url) : new URL(`http://dummy${url}`);
    ...
    sensitiveParams.forEach((param) => {
      if (urlObj.searchParams.has(param)) {
        ...
        const masked = value && value.length > 4
    ...
    return url.includes('://') ? sanitized : sanitized.replace('http://dummy', '');
  } catch (error) {
    // If URL parsing fails, do basic string replacement
    return url.replace(/([?&])(api_key|apiKey|token|secret|password|key)=([^&]+)/gi, '$1$2=***REDACTED***');
  }
}
