    ...
    schema: {
      ...
      properties: {
        ...
        retryAfter: {
          type: "number",
          description:
            "Seconds until quota resets (only present when quota exceeded)",
          example: 3600,
        },
        windowStart: {
          type: "string",
  ...
  async getStatus(@Req() request: Request) {
    try {
      ...
      const maxRequestsPerWindow =
        this.rateLimitService.getMaxRequestsPerWindow();
      const windowDurationHours =
        this.rateLimitService.getWindowDurationHours();

      // Calculate percentage of quota remaining
      const percentage = Math.round(
        (status.remaining / maxRequestsPerWindow) * 100,
      );
