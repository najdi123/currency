...
import { CIRCUIT_BREAKER } from '../constants/navasan.constants';
 ...
 */
@Injectable()
export class NavasanCircuitBreakerService {
  private readonly logger = new Logger(NavasanCircuitBreakerService.name);
  ...
  canProceed(): boolean {
    ...
    switch (this.state) {
      ...
      case CircuitState.HALF_OPEN:
        ...
        this.logger.warn('Circuit breaker HALF_OPEN limit reached - rejecting request');
        return false;
  ...
  recordSuccess(): void {
    ...
    switch (this.state) {
      ...
      case CircuitState.OPEN:
        ...
        this.logger.warn('Success recorded while circuit was OPEN - closing circuit');
        this.close();
        break;
