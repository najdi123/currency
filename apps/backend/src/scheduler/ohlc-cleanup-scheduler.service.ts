...
export class OhlcCleanupSchedulerService {
  ...
  constructor(
    ...
    this.isEnabled = this.configService.get<string>('OHLC_CLEANUP_ENABLED', 'true') === 'true';

    if (!this.isEnabled) {
      this.logger.warn('⚠️  OHLC cleanup scheduler is DISABLED');
    } else {
      this.logger.log('✅ OHLC cleanup scheduler is ENABLED - runs daily at 2:00 AM UTC');
    }
  }
  ...
  async cleanupOldOhlcSnapshots() {
    ...
    try {
      ...
      for (const [timeRange, retentionDays] of Object.entries(this.retentionPolicies)) {
        ...
        try {
          ...
          if (deletedCount > 0) {
            ...
            this.logger.log(`✅ No old ${timeRange} OHLC snapshots to clean up`);
          }
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : String(error);
          this.logger.error(`❌ Failed to clean up ${timeRange} OHLC snapshots: ${errorMessage}`);
        }
      }
   ...
   */
  async triggerManualCleanup(): Promise<{ success: boolean; deletedCount: number; message: string }> {
  ...
  async getOhlcStats(): Promise<{
    ...
    try {
      ...
      for (const timeRange of Object.keys(this.retentionPolicies)) {
        const count = await this.ohlcSnapshotModel.countDocuments({ timeRange });
