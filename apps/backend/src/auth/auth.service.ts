...
export class AuthService {
  ...
  private async handleFailedLogin(userId: string): Promise<void> {
    ...
    if (attempts >= AUTH_CONSTANTS.MAX_FAILED_ATTEMPTS) {
      ...
      lockoutUntil.setMinutes(
        lockoutUntil.getMinutes() + AUTH_CONSTANTS.LOCKOUT_DURATION_MINUTES,
      );
  ...
  async login(dto: LoginDto, ipAddress?: string, userAgent?: string) {
    ...
    const hashToCompare =
      user?.passwordHash || (await bcrypt.hash("dummy", 10));
    ...
    if (user.status !== UserStatus.ACTIVE) {
      this.logger.warn(
        `Login attempt for inactive account: ${dto.email} (status: ${user.status})`,
      );
  ...
  async generateRefreshToken(
    ...
    try {
      ...
      this.logger.error(
        `Failed to generate refresh token for user ${userId}: ${error.message}`,
        error.stack,
      );
  ...
  async refreshAccessToken(
    ...
    try {
      ...
      if (tokenDoc.expiresAt < new Date()) {
        this.logger.warn(
          `Expired refresh token attempted for user: ${tokenDoc.userId}`,
        );
      ...
      if (user.status !== UserStatus.ACTIVE) {
        this.logger.warn(
          `Token refresh attempted for inactive user: ${user.email} (status: ${user.status})`,
        );
  ...
  async revokeAllRefreshTokens(userId: string): Promise<{ message: string }> {
    try {
      ...
      this.logger.log(
        `Revoked ${result.deletedCount} refresh tokens for user: ${userId}`,
      );
      return {
      ...
      this.logger.error(
        `Failed to revoke all refresh tokens for user ${userId}: ${error.message}`,
        error.stack,
      );
  ...
  async changePassword(
    ...
    await this.auditService.logPasswordChange(userId, user.email, ipAddress);
