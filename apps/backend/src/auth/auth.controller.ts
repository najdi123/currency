...
export class AuthController {
  ...
  @ApiResponse({ status: 429, description: 'Too many requests - Rate limit exceeded (5 attempts per minute)' })
  login(
    @Body() dto: LoginDto,
    @Ip() ipAddress?: string,
    @Headers('user-agent') userAgent?: string,
  ...
  @ApiResponse({ status: 429, description: 'Too many requests - Rate limit exceeded (3 attempts per minute)' })
  changePassword(
  ...
  @ApiResponse({ status: 200, description: 'Verification email sent if address exists' })
  @ApiResponse({ status: 429, description: 'Too many requests - Rate limit exceeded (3 attempts per minute)' })
  resendVerification(
